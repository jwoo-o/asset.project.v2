...
@Repository
public class ApproverGroupRepositorySupport extends QuerydslRepositorySupport {
    ...
    public ApproverGroupRepositorySupport(JPAQueryFactory jpaQueryFactory) {
        super(ApproverGroup.class);
    ...
    public Map<String, Object> findAllBySearch(SearchRequestDto dto) {
        ...
        List<Long> codes = jpaQueryFactory.select(approverGroup.approverGroupCode)
                .from(approverGroup).innerJoin(organization).on(approverGroup.orgCode.eq(organization.orgCode))
                .where(builder).orderBy(approverGroup.createdAt.desc()).limit(dto.getTotal()).offset(dto.getOffset()).fetch();
        ...
        if (!codes.isEmpty()) {
            list = jpaQueryFactory.select(Projections.fields(ApproverGroupResponseDto.class,
                    ...
                    .from(approverGroup).where(approverGroup.approverGroupCode.in(codes.stream().limit(dto.getLimit()).collect(Collectors.toList()))).fetch();
            ...
            List<ApproverDetail> details = jpaQueryFactory.select(approverDetail).from(approverDetail)
                                                        .where(approverDetail.approverGroupCode
                                                                .in(list.stream()
                                                                        .map(approverGroupResponseDto -> approverGroupResponseDto.getApproverGroupCode())
            ...
            for(ApproverGroupResponseDto responseDto : list) {
                responseDto.setApprover(
                        details.stream()
                                ...
                                .map(detail -> detail.getUserName()).collect(Collectors.joining(" > ")));
                responseDto.setCc(
                        details.stream()
                                .filter(detail ->
