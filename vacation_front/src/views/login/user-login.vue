    ...
    <div class="login-div">
      ...
      >
        ...
        <div class="title-container">
          <h3 class="title">Joeunins 로그인 페이지</h3>
        </div>
...
import { api_user_pwd_edit } from '@/api/user/user'
...
export default {
  ...
  data() {
    ...
    const validatePasswordCheck = (rule, value, callback) => {
      ...
      } else if (value === this.loginForm.userId) {
    ...
    const validateRePasswordCheck = (rule, value, callback) => {
      if (value === '') {
        ...
        this.passwordForm.userId = this.loginForm.userId
        callback()
      }
    }
    return {
      ...
      loginRules: {
        userId: [{ required: true, trigger: 'blur', validator: validateAdminId }],
      ...
      this.$refs.loginForm.validate(async valid => {
        if (valid) {
          ...
          await this.$store.dispatch('user/login', data)
            .then(() => {
              this.$router.push({ path: this.redirect || '/user-home', query: this.otherQuery })
              // location.reload()
              this.loading = false
            })
            .catch((data) => {
    ...
    handleChange() {
      this.$refs.passwordForm.validate(async valid => {
        if (valid) {
          ...
          await api_user_pwd_edit(data).then(() => {
            this.$message({
